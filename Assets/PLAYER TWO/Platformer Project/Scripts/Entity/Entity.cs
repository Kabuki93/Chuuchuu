...
namespace PLAYERTWO.PlatformerProject
{
	...
	public abstract class Entity<T> : Entity where T : Entity<T>
	{
		...
		protected virtual void HandleGround()
		{
			...
			var validAngle = colliding && Vector3.Angle(hit.normal, worldUp) <= controller.slopeLimit;
			var landing = colliding && movingTowardGround && (validAngle || rotateToGround);
		...
		protected virtual void UpdateGroundRotation()
		{
			...
			var groundBellow = Physics.Raycast(position, -transform.up, out var hit,
				height, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);

			if (groundBellow)
			{
				var rotation = Quaternion.FromToRotation(transform.up, hit.normal);
				transform.rotation = rotation * transform.rotation;
				velocity = rotation * velocity;
			}
		}
		...
		{
			if (!isGrounded || !rotateToGround || gravityField || platform)
				return;

			var steepGround = groundAngle >= controller.slopeLimit;
		...
		public virtual void FaceDirection(Vector3 direction, float degreesPerSecond, bool adjustUp = true)
		{
			if (direction != Vector3.zero)
			{
				if (adjustUp)
